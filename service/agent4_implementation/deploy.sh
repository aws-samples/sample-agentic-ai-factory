...
STACK_NAME="${PROJECT_NAME}-agent4-${ENVIRONMENT}"

echo "🚀 Deploying Agent 4 - Implementation"
...
aws cloudformation deploy \
    ...
    --region $REGION \
    --tags Environment=$ENVIRONMENT Project=$PROJECT_NAME Agent=agent4-implementation
...
ECR_REPO="bedrock-agentcore-agent4_implementation_${ENVIRONMENT}"
...
printf "%s\n%s\nno\nno\nno\nno\n" "$EXECUTION_ROLE_ARN" "$ECR_URI" | agentcore configure \
    --entrypoint agent4.py \
    --name agent4_implementation_${ENVIRONMENT} \
...
agentcore launch --agent agent4_implementation_${ENVIRONMENT} --auto-update-on-conflict
